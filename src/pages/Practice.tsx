...
import { Brain, Target, Clock, CheckCircle, BookOpen, Zap, TrendingUp } from "lucide-react";

const Practice = () => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const [isAuthModalOpen, setIsAuthModalOpen] = useState(false);
  const [activeMode, setActiveMode] = useState('quiz'); // 'quiz', 'assessment', 'problems'
      ...
      <div className="container mx-auto px-6 py-12 max-w-7xl">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold mb-6">
            Practice <span className="bg-gradient-hero bg-clip-text text-transparent">Hub</span>
          </h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Master Ethiopian curriculum through AI-powered practice sessions, quizzes, and comprehensive assessments
          </p>
        </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="bg-muted p-1 rounded-lg">
            {[
              { id: 'quiz', label: 'Smart Quiz', icon: Zap },
              { id: 'assessment', label: 'Assessment', icon: TrendingUp },
              { id: 'problems', label: 'Practice Problems', icon: Target }
            ].map((mode) => {
        ...
        {activeMode === 'assessment' && (
          <InteractiveAssessment 
            title="Ethiopian Curriculum Assessment"
            subject="Mathematics"
            topic="Algebra"
            questions={[]}
            timeLimit={60}
